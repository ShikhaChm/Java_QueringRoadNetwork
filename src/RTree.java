...
public class RTree extends Tree {
	...
	public RTree(List<TrajectoryObject> trajectoryObjectList) {
		long startTime = System.nanoTime();
		if(trajectoryObjectList.size()==0) root=null;
		else {
			root = new RTreeNode(trajectoryObjectList.get(0));
			for(int i=1; i < trajectoryObjectList.size();i++) {
				RTreeNode returnNode = ((RTreeNode)root).addPoint(trajectoryObjectList.get(i));
				if(returnNode!=null) root = ((RTreeNode)root).getMinimumBoundingNode(returnNode);
			}
		}
		long endTime = System.nanoTime();
		System.out.println("RTree Creation Time: "+(endTime-startTime)/1000);
	}
